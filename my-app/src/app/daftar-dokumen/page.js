...
export default function DaftarDokumen() {
  ...
  useEffect(
    () => {
      const fetchDokumen = async () => {
        try {
          const url = searchQuery ? `${process.env.NEXT_PUBLIC_BASE_URL}/daftarDokumen/searchDokumen/?q=${encodeURIComponent(searchQuery)}` : `${process.env.NEXT_PUBLIC_BASE_URL}/daftarDokumen/daftarDokumen/`;
  ...
  const handleDelete = async (index, dokumenId) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/daftarDokumen/${dokumenId}/delete/`, {
        method: "DELETE",
      });
    ...
    <div className={styles.mainContainer}>
      ...
      <div className={styles.container}>
        ...
        <table className={styles.table}>
          <thead>
            <tr className={styles.header}>
              <th onClick={() => handleSort("id")}>Kode {sortColumn === "id" ? (sortOrder === "asc" ? "↑" : "↓") : ""}</th>
              <th>Judul</th>
              <th>Klien</th>
              <th>Jenis Dokumen</th>
              <th>Action</th>
          ...
          <tbody>
            {currentDokumen.map((dokumen, index) => (
              <tr key={dokumen.id} className={index + indexOfFirstDokumen === deletingIndex ? styles.fadeOut : ""}>
                <td>{dokumen.id}</td>
                <td>{dokumen.survey_name}</td>
                <td>{dokumen.client_name}</td>
                <td>{dokumen.doc_type}</td>
                <td className={styles.actions}>
                  <button onClick={() => handleDetail(dokumen.id, dokumen.doc_type)} className="text-gray-800 hover:text-gray-600 transition-colors">
        ...
        <div className={styles.pagination}>
          {currentPage > 1 && <button onClick={() => setCurrentPage(currentPage - 1)}>&lt; Kembali</button>}
          {currentPage < totalPages && <button onClick={() => setCurrentPage(currentPage + 1)}>Selanjutnya &gt;</button>}
        </div>
      </div>
      {showDeleteModal && (
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            ...
            <button onClick={() => handleDelete(currentDokumenIndex, dokumen[currentDokumenIndex].id)} className={styles.confirmButton}>
              Ya
            </button>
