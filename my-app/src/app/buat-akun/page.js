...
export default function BuatAkun() {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const res = await fetch('http://localhost:8000/api/buatAkun/', {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="buatAkun-container">
      ...
      <h2 className="buatAkun-heading">Buat Akun</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="buatAkub-btn">Simpan</button>
      </form>
      ...
      {showNotification && (
        <div className={`notification ${success ? 'success' : 'failure'}`}>
          ...
          (
            <div>
              <img src="/error-icon.png" alt="Error Icon" className="icon" />
              <p>Isi password sesuai ketentuan!</p>
              <button onClick={() => setShowNotification(false)}>Coba lagi</button>
            </div>
          )}
        </div>
