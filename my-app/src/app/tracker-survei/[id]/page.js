...
export default function SurveyTrackerDetail({ params }) {
  ...
  useEffect(() => {
    const verifyUser = async () => {
      ...
      try {
        const roleEndpoints = [
          { url: `${process.env.NEXT_PUBLIC_BASE_URL}accounts/check_role_administrasi/`, role: "Administrasi" },
          { url: `${process.env.NEXT_PUBLIC_BASE_URL}accounts/check_role_logistik/`, role: "Logistik" },
          { url: `${process.env.NEXT_PUBLIC_BASE_URL}accounts/check_role_pengendalimutu/`, role: "Pengendali Mutu" },
