...
export default function BuatSurvei() {
  ...
  const fetchDaerah = async (param) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}survei/lokasi?param=${param}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchKlien = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}klien/`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const data = await response.json();
      setKlien(data.results); // Set hasil ke daftar klien
    } catch (error) {
